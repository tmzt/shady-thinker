// Batched Add + RMSNorm for Qwen3.5 GPTQ prefill:
//   hidden[row,i] += addend[row,i]
//   output[row,i]  = hidden[row,i] * rms * (1 + weight[i])
// Uses (1 + w) norm scaling (contrast with batched_add_rmsnorm which uses direct w).
...
fn main(
    ...
    // Write normalized output with (1 + w) scaling
    i = tid;
    while (i < N) {
        let w = unpack_bf16(weight[i / 2u], i % 2u);
        output[base + i] = hidden[base + i] * rms * (1.0 + w);
        i += 256u;
    }
}
