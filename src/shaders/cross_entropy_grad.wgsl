...
var<workgroup> wg_temp: array<f32, 256u>;

@compute @workgroup_size(256, 1, 1)
fn main(@builtin(local_invocation_id) lid: vec3<u32>) {
    let local_id = lid.x;
    let vocab_size = params.vocab_size;
    let tgt = params.target_token;
    ...
    wg_temp[local_id] = local_max;
    workgroupBarrier();

    // Tree reduction for max.
    for (var stride = 128u; stride > 0u; stride >>= 1u) {
        if local_id < stride {
            wg_temp[local_id] = max(wg_temp[local_id], wg_temp[local_id + stride]);
        }
        workgroupBarrier();
    }
    let max_val = wg_temp[0];
    workgroupBarrier();
    ...
    wg_temp[local_id] = local_sum;
    workgroupBarrier();

    // Tree reduction for sum.
    for (var stride = 128u; stride > 0u; stride >>= 1u) {
        if local_id < stride {
            wg_temp[local_id] = wg_temp[local_id] + wg_temp[local_id + stride];
        }
        workgroupBarrier();
    }
    let sum_exp = wg_temp[0];
    workgroupBarrier();
    ...
    if local_id == 0u {
        let loss = -(logits[tgt] - max_val - log(sum_exp));
        loss_out[0] = loss;
    }
}
