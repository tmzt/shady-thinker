// GPTQ INT4 Matrix-Matrix Multiply with 4-thread lane parallelism.
// Extends gptq_matvec_4t with a sequence (batch) dimension in workgroup_id.y.
...
fn main(
    ...
    if (lane == 0u) {
        output[out_base + col] = scratch[tid] + scratch[tid + 1u];
    }
}
