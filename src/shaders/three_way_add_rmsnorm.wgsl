// Three-way add + RMSNorm: val = a[i] + b[i] + c[i]
// hidden_out[i] = val (raw sum), normed[i] = val * rms * (1 + weight[i])
...
fn main(@builtin(local_invocation_id) lid: vec3<u32>) {
    ...
    // Phase 1: Compute three-way sum, write raw sum, accumulate sum of squares.
    var sum_sq: f32 = 0.0;
    var i = tid;
    while (i < N) {
        let val = a[i] + b[i] + c[i];
    ...
    while (i < N) {
        ...
        normed[i] = hidden_out[i] * rms * (1.0 + w);
        i += 256u;
    }
}
